...
const ProfileField = ({ label, value, icon: Icon, disabled, onChange, helperText }) => {
  return (
    <div className="group">
        ...
        <input
          ...
          className={`
            ...
            ${disabled 
              ...
              : "bg-white text-gray-900 border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10"
            }
          `}
        />
      </div>
      ...
      {helperText && disabled && (
        <p className="mt-1.5 ml-1 text-xs text-gray-400">{helperText}</p>
      )}
    </div>
  );
};

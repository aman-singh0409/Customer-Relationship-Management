...
import { MoreHorizontal, Search, Filter, Package } from "lucide-react";
import ProductDetailsModal from "./ProductDetailsModal";

const ProductTable = ({ products, isLoading }) => {
  // Removed: useAuthStore, axios, api (not needed for view-only)
  
  const [searchQuery, setSearchQuery] = useState("");
  
  // Modal State - Only for View Details
  const [selectedProduct, setSelectedProduct] = useState(null); 
  
  // Removed: Edit/Delete states (productToEdit, isEditModalOpen)
  // Removed: Role checks (canEdit, canDelete)
  ...
  return (
    <>
                  ...
                  <tr key={product._id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button onClick={() => setSelectedProduct(product)} className="p-2 text-slate-400 hover:text-blue-600">
      ...
      {/* Removed: EditProductModal component */}
    </>
  );
};
